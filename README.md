...
- 🎛️ **QQ 命令**：总结 / 邮件列表 / 邮件详情 / 手动扫描 / 重新总结
- 🖥️ **WebUI 邮件看板**（AstrBot ≥ 4.24.1）：在 AstrBot 插件详情页打开「邮件看板」，网页上查看邮件列表 / 详情 / 汇总报告，一键触发扫描，一键重新总结
- 🔄 **重新总结**：网页按钮或 QQ 命令 `/重新总结` 把「未总结/上次失败的邮件」重新 LLM 分析一遍；加「全部」参数则强制所有邮件（无论是否已总结）重新分析并覆盖旧结果，完成后自动重新生成汇总报告
...
- **重新总结**：下拉选择「仅未总结/失败」或「全部邮件」，点击一键重新分析；全部模式会强制覆盖所有旧分析结果，完成后可点击「生成汇总」查看最新报告（后台执行）
...
| `/重新总结` | 把「未总结/上次失败的邮件」重新逐封 LLM 分析 |
| `/重新总结 全部` | 强制所有邮件（无论是否已总结）重新分析并覆盖旧结果 |
| `/帮助` | 显示可用命令 |
