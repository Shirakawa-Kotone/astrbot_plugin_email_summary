...
- 🔔 **高优即时提醒**：扫描到 High/Medium 优先级邮件时立即向目标 QQ 发送提醒（需配置 `target_qq` 且 `tag_notifications_enabled=true`）
- 🎛️ **QQ 命令**：总结 / 邮件列表 / 邮件详情 / 手动扫描 / 重新总结 / 标签管理
- 🏷️ **邮件标签系统**：支持通过 QQ 命令或网页为邮件添加自定义标签
  - **"已完成"**：标记该邮件已完成，在汇总报告和高优列表中隐藏
  - **"代办"**：强制提升为中优先级，即使 LLM 判定为 Low 也会按中优先级展示
  - 其他自定义标签：用于筛选和展示，不影响优先级
- 🖥️ **WebUI 邮件看板**（AstrBot ≥ 4.24.1）：在 AstrBot 插件详情页打开「邮件看板」，网页上查看邮件列表 / 详情 / 汇总报告，一键触发扫描，一键重新总结，支持标签管理
- 🔄 **重新总结**：网页按钮或 QQ 命令 `/重新总结` 把「未总结/上次失败的邮件」重新 LLM 分析一遍；加「全部」参数则强制所有邮件（无论是否已总结）重新分析并覆盖旧结果，完成后自动重新生成汇总报告
- ⏰ **调度策略**：间隔模式（每 N 小时）或每日定时模式
- 💾 **持久化**：分析结果存于 `data/analysis/{uid}.json`，已处理邮件记录于 `data/state.json`，标签存于 `data/tags.json`
- 🏷️ **标签持久化**：所有标签操作结果持久化在 `data/tags.json`，重启不丢失
...
| `/标签 邮件编号 标签名` | 为指定邮件添加标签（如 `/标签 1 已完成`） |
| `/标签 列表` | 查看所有有标签的邮件 |
| `/去标签 邮件编号 标签名` | 移除指定邮件的指定标签 |
| `/清标签 邮件编号` | 清除指定邮件的所有用户标签 |
| `/帮助` | 显示可用命令 |

### 标签系统

标签支持通过 QQ 命令和网页看板使用。标签分为两类：

**系统自动标签**（LLM 分析时自动生成，只读）：
- `重要`、`紧急`、`含附件`、`含链接`、`需行动` 等

**用户自定义标签**（通过命令或网页添加，可管理）：

| 特殊标签 | 效果 |
|---------|------|
| `已完成` | 该邮件在汇总报告和高优列表中隐藏，邮件列表显示 ✅ 标记 |
| `代办` | 即使 LLM 判定为 Low 优先级，也会按中优先级展示 |
| 其他（如 `重审`、`优先处理`） | 仅用于筛选和展示，不影响优先级 |

**网页看板**：点击邮件查看详情，在详情页可以直接添加/移除标签（点击标签后鼠标悬停显示 ✕ 可移除）。
...
| `tag_notifications_enabled` | `true` | 扫描到 High/Medium 邮件时立即通知目标 QQ（需同时配置 `target_qq`） |
